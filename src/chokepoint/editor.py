...
class ArsenalEditor:
    ...
    def to_python(self, graph: Graph | None = None) -> str:
        ...
        since positions are fixed. If ``graph`` is given, also emit a
        ``build_topology()`` so a custom map survives a relaunch.
        """
        ...
        lines = [
            ...
            "",
            "Drop-in build_loadout (+ build_gates/limiters/topology): F5 or relaunch.",
            '"""',
            "from __future__ import annotations",
            "",
        ...
        if graph is not None:
            lines.append(f"from {__package__}.maps import build_graph")
        ...
        if self.limiters:
            ...
            lines.append("    return []")
        if graph is not None:
            # persist the custom topology so a designed map survives a relaunch
            lines += ["", "", "def build_topology():", "    coords = {"]
            lines += [f"        {nid!r}: ({n.x!r}, {n.y!r})," for nid, n in graph.nodes.items()]
            lines += ["    }", "    edges = ["]
            lines += [f"        ({src!r}, {dst!r})," for src, dst in graph.edges()]
            lines += ["    ]"]
            slots = ", ".join(f"({x!r}, {y!r})" for x, y in graph.slots)
            lines.append(f"    return build_graph({graph.name!r}, coords, edges, "
                         f"{graph.source!r}, {graph.sink!r}, [{slots}])")
        lines.append("")
        return "\n".join(lines)
